...
class PduRaspberrypiDcProviderExample(BaseTestCase):
    ...
    def test_dc_power_reset(self):
        ret = self._dc.get_dc_power_state()
        if (ret == True):
            self._log.info("System Testing Passed For Rebooting Platform")
        else:
            self._log.error("System Testing FAILED For Rebooting Platform")
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PduRaspberrypiDcProviderExample.main() else Framework.TEST_RESULT_FAIL)
