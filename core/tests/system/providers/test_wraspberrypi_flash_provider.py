...
class PiFlashProviderExample(BaseTestCase):
    def __init__(self, test_log, arguments, cfg_opts):
        super(PiFlashProviderExample, self).__init__(test_log, arguments, cfg_opts)
    ...
    def test_flash_image(self):
        ret = self._flash.chip_identify()
        if (ret == True):
            ret = self._flash.flash_image("xeon-d.bin")
            if (ret == True):
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PiFlashProviderExample.main() else Framework.TEST_RESULT_FAIL)
