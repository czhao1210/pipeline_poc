...
class RedfishDCProviderExample(BaseTestCase):
    def __init__(self, test_log, arguments, cfg_opts):
        super(RedfishDCProviderExample, self).__init__(test_log, arguments, cfg_opts)
        flash_cfg = cfg_opts.find(DcPowerControlProvider.DEFAULT_CONFIG_PATH)
        self._dc = ProviderFactory.create(flash_cfg, test_log)
    ...
    def test_dc_power_reset(self):
        ret=self._dc.dc_power_reset()
        if(ret == True):
            self._log.info("System Testing Passed For DC Power Reset")
        else:
            self._log.error("Ssytem Testing FAILED For DC Power Reset")
    ...
    def execute(self):
        ...
        #self.test_get_dc_power_state(output=False)
        # time.sleep(4)
        self.test_dc_power_on()
        #self.test_get_dc_power_state(output=True)
        # self.test_dc_power_reset()
        # return True

if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if RedfishDCProviderExample.main() else Framework.TEST_RESULT_FAIL)
