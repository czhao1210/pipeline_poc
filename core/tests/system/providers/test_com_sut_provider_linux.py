...
class TestSuites(object):
    def setup_class(self):
        ...
        self.comsutoscfg = ConfigurationHelper.get_provider_config(sut=sut, provider_name='sut_os')

        self.acpower = ProviderFactory.create(acpwrcfg, log)  # type:Soundwave2kAcProvider
        self.com_sutos = ProviderFactory.create(self.comsutoscfg, log)  # type: ComSutOsProvider

    def test_shutdown(self):
        self.com_sutos.update_configuration(update_cfg_opts=self.comsutoscfg, update_log=log)
