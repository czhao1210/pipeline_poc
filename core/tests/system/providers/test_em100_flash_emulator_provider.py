...
@pytest.mark.em100
class Em100FlashEmulatorProviderSystemTests(BaseSystemTest):
    """Live system tests for Em100FlashEmulatorProvider"""
    # obj: Em100FlashEmulatorProvider  # Uncomment this line for IDE type hinting (not supported in Python2)

    CLASS_UNDER_TEST = Em100FlashEmulatorProvider
    ...
    def test_multiple_devices(self):
        ...
        pch_data = self.obj.read(test_address, test_str_length, FlashDevices.PCH)
        bmc_data = self.obj.read(test_address, test_str_length, FlashDevices.BMC)
        ...
        pch_magic = bytearray(os.urandom(test_str_length))
        bmc_magic = bytearray(os.urandom(test_str_length))
        self.obj.write(test_address, pch_magic, FlashDevices.PCH)
        self.obj.write(test_address, bmc_magic, FlashDevices.BMC)

        time.sleep(2.0)
        # read back the values written
        try:
            read_back_pch = self.obj.read(test_address, test_str_length, FlashDevices.PCH)
            read_back_bmc = self.obj.read(test_address, test_str_length, FlashDevices.BMC)
        ...
        # (attempt to) restore original values
            ...
            self.obj.write(test_address, pch_data, FlashDevices.PCH)
            self.obj.write(test_address, bmc_data, FlashDevices.BMC)
    ...
    def test_start_stop(self):
        ...
        time.sleep(2.0)
        self.obj.start(FlashDevices.PCH)
