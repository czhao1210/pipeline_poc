...
import pytest
from dtaf_core.providers.internal.pi_dc_provider import PiDcProvider
...
class TestPiDcProvider(object):
    ...
    def test_ac_power_off(driver_cfg, provider, driver, timeout, return_state, state, mocker, monkeypatch):
        ...
        mocker.patch.object(ConfigurationHelper, 'get_driver_config', return_value=None)
        mocker.patch.object(DriverFactory, 'create', new=_Driver)
        assert PiDcProvider(cfg_opts,log).dc_power_off(timeout) is state
    ...
    def test_ac_power_off_exception(driver_cfg, provider,driver,timeout,return_state,state, mocker, monkeypatch):
        with pytest.raises(DriverIOError):
            ...
            PiDcProvider(cfg_opts, log).dc_power_off(timeout)
    ...
    def test_ac_power_on(driver_cfg, provider, driver, timeout, return_state, state, mocker, monkeypatch):
        ...
        mocker.patch.object(ConfigurationHelper, 'get_driver_config', return_value=None)
        mocker.patch.object(DriverFactory, 'create', new=_Driver)
        assert PiDcProvider(cfg_opts,log).dc_power_on(timeout) is state
    ...
    def test_ac_power_on_exception(driver_cfg, provider,driver,timeout,return_state,state, mocker, monkeypatch):
        with pytest.raises(DriverIOError):
            mocker.patch.object(ConfigurationHelper, 'get_driver_config', return_value=None)
            driver.return_value=_Driver_exception()
            PiDcProvider(cfg_opts, log).dc_power_on(timeout)
    ...
    def test_get_dc_power_state(driver_cfg, provider, driver,return_state, state, mocker, monkeypatch):
        with pytest.raises(DriverIOError):
            mocker.patch.object(ConfigurationHelper, 'get_driver_config', return_value=None)
            mocker.patch.object(DriverFactory, 'create', new=_Driver)
            raise DriverIOError('if command execution failed in driver')
        mocker.patch.object(ConfigurationHelper, 'get_driver_config', return_value=None)
        mocker.patch.object(DriverFactory, 'create', new=_Driver)
        assert PiDcProvider(cfg_opts,log).get_dc_power_state() is state
    ...
    @mock.patch('dtaf_core.lib.configuration.ConfigurationHelper.get_driver_config',return_value=None)
    @pytest.mark.parametrize('timeout, return_state,state',[(5,None,Exception)])
    def test_get_dc_power_state_exception(driver_cfg, provider,driver,timeout,return_state,state, mocker, monkeypatch):
        with pytest.raises(DriverIOError):
            mocker.patch.object(ConfigurationHelper, 'get_driver_config', return_value=None)
            driver.return_value=_Driver_exception()
            PiDcProvider(cfg_opts, log).get_dc_power_state()
            raise DriverIOError('pi get dc power state error')
    ...
    def test_dc_power_reset(driver_cfg, provider, driver,return_state, state, mocker, monkeypatch):
        with pytest.raises(DriverIOError):
            mocker.patch.object(ConfigurationHelper, 'get_driver_config', return_value=None)
            mocker.patch.object(DriverFactory, 'create', new=_Driver)
            raise DriverIOError('if command execution failed in driver')
        mocker.patch.object(ConfigurationHelper, 'get_driver_config', return_value=None)
        mocker.patch.object(DriverFactory, 'create', new=_Driver)
        assert PiDcProvider(cfg_opts,log).dc_power_reset() is state
    ...
    @mock.patch('dtaf_core.lib.configuration.ConfigurationHelper.get_driver_config',return_value=None)
    @pytest.mark.parametrize('timeout, return_state,state',[(5,None,Exception)])
    def test_dc_power_reset_exception(driver_cfg, provider,driver,timeout,return_state,state, mocker, monkeypatch):
        with pytest.raises(DriverIOError):
            mocker.patch.object(ConfigurationHelper, 'get_driver_config', return_value=None)
            driver.return_value=_Driver_exception()
            PiDcProvider(cfg_opts, log).dc_power_reset()
            raise DriverIOError('pi get dc power reset error')
