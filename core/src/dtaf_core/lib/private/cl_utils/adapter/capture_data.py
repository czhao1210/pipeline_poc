...
class OutputCapture(object):
    ...
    def clean(self):
        """
        cleanup the data in the buffer
        :return:  None
        """
        ret = RET_TEST_FAIL
        try:
            ...
            print('clean error from {0}'.format(ex))
            self.__capture_list = None
            ret = RET_ENV_FAIL
        finally:
            self.__lock.release()
    ...
    def return_on_detection(self):
        """
        get the flag of return_on_detection safely
        :return: the flag of return_on_detection
        """
        ret = False
        try:
            self.__lock.acquire()
    ...
    def get_result(self):
        """
        get the result of captured data
        :return: str of captured data
        """
        ret = ([], False)
        try:
            self.__lock.acquire()
