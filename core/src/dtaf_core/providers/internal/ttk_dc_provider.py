...
class TTKDcProvider(DcPowerControlProvider,TTKDriver):
     def __init__(self,cfg_opts,log):
          super(TTKDcProvider, self).__init__(cfg_opts, log)

     def __enter__(self):
          return super(TTKDcProvider, self).__enter__()

     def __exit__(self, exc_type, exc_val, exc_tb):
          super(TTKDcProvider, self).__exit__(exc_type, exc_val, exc_tb)
     ...
     def dc_power_on(self,timeout=None):
          ...
          :return: True or None
          :exception
            ttk Error: TTK Library Throws Error.
          """
          if timeout is None:
             timeout = self._config_model.poweron_timeout
          driver_cfg = ConfigurationHelper.get_driver_config(provider=self._cfg,driver_name=r"ttk")
          with DriverFactory.create(cfg_opts=driver_cfg, logger=self._log) as sw:
               try:
                    if sw.dc_power_on(timeout):
                        return True
               except Exception as ex:
                    self._log.error("TTK DC power On error:{}".format(ex))
                    raise
     ...
     def dc_power_off(self,timeout=None):
          ...
          with DriverFactory.create(cfg_opts=driver_cfg, logger=self._log) as sw:
               try:
                    ...
                    self._log.error("TTK DC power Off error:{}".format(ex))
                    raise
