...
from dtaf_core.drivers.internal.pduapc_driver import PduapcDriver

class PduapcAcProvider(AcPowerControlProvider, PduapcDriver):
    """
    This class is used to provide interfaces for the ac power controlling on PI Control box.
    """

    def __init__(self, cfg_opts, log):
        super(PduapcAcProvider, self).__init__(cfg_opts, log)

    def __enter__(self):
        return super(PduapcAcProvider, self).__enter__()

    def __exit__(self, exc_type, exc_val, exc_tb):
        super(PduapcAcProvider, self).__exit__(exc_type, exc_val, exc_tb)
    ...
    def ac_power_off(self, timeout=None):
        ...
        with DriverFactory.create(cfg_opts=driver_cfg, logger=self._log) as s_w:
            try:
                if s_w.ac_power_off(timeout):
                    return True
            except Exception as ex:
                self._log.error("PDU Telnet ac power off error:{}".format(ex))
                raise DriverIOError(ex)
