...
class BaseBiosSetupMenuProvider(bios_menu.BiosSetupMenuProvider):
    def __init__(self, cfg_opts, log):
        super(BaseBiosSetupMenuProvider, self).__init__(cfg_opts, log)
        self.__width = self._config_model.width
        self.__height = self._config_model.height
        self.__logger = log
        self.buffer_name = 'setupment_buffer'
        driver_cfg = None
        ...
        self.__driver = DriverFactory.create(driver_cfg, logger=log)
        self.__driver.register(buffer_name=self.buffer_name)
        self.__adapter = _ContextInstance.get(
            serial_service=self.__driver,
            is_host_service=False, logger=log, buffer_name=self.buffer_name, width=self.__width, high=self.__height).data_adapter[
            'setup_menu']
        self.__data_adapter = _ContextInstance.get(self.__driver, logger=self.__logger, buffer_name=self.buffer_name,
                                                   width=self.__width, high=self.__height
                                                   ).data_adapter['entry_menu']
