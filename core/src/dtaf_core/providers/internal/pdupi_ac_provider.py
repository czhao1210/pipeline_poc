...
class PdupiAcProvider(AcPowerControlProvider, PdupiDriver):
    """
    This class is used to provide interfaces for the ac power controlling on PI Control box.
    """
    def __init__(self, cfg_opts, log):
        super(PdupiAcProvider, self).__init__(cfg_opts, log)
        
    def __enter__(self):
        return super(PdupiAcProvider, self).__enter__()

    def __exit__(self, exc_type, exc_val, exc_tb):
        super(PdupiAcProvider, self).__exit__(exc_type, exc_val, exc_tb)
    ...
    def ac_power_on(self,username=None,password=None,channel=None,timeout=None):
        ...
        with DriverFactory.create(cfg_opts=driver_cfg, logger=self._log) as sw:
            try:
                if sw.ac_power_on(username,password,channel,timeout):
                    return True
                else:
                    return False
    ...
    def ac_power_off(self,username=None,password=None,channel=None,timeout=None):
        ...
        with DriverFactory.create(cfg_opts=driver_cfg, logger=self._log) as sw:
            try:
                if sw.ac_power_off(username,password,channel,timeout):
                    return True
                else:
                    return False
            except Exception as ex:
                self._log.error("pdu ac power off error:{}".format(ex))
                raise RPiError(ex)
    ...
    def get_ac_power_state(self,username=None,password=None,channel=None):
        ...
        with DriverFactory.create(cfg_opts=driver_cfg, logger=self._log) as sw:
            try:
                if sw.get_ac_power_state(username,password,channel):
                    return True
                else:
                    return False
            except Exception as ex:
    ...
    def set_username_password(self,username=None,password=None,channel=None):
        # type: (None) -> str
        """
        ...
        with DriverFactory.create(cfg_opts=driver_cfg, logger=self._log) as sw:
            try:
                if sw.set_username_password(channel,username,password):
                    return True
            except Exception as ex:
                self._log.error("setting username and password resulted in error:{}".format(ex))
                raise RPiError(ex)
    ...
    def reset_username_password(self,channel=None,masterkey=None):
        ...
        with DriverFactory.create(cfg_opts=driver_cfg, logger=self._log) as sw:
            try:
                if sw.reset_username_password(channel,masterkey):
                    return True
            except Exception as ex:
                self._log.error("pdupi resetting username_password resulted in error:{}".format(ex))
                raise RPiError(ex)
