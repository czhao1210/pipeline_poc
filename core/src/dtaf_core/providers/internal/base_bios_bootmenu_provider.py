...
class BaseBiosBootmenuProvider(BiosBootMenuProvider):
    def __init__(self, cfg_opts, log):
        super(BaseBiosBootmenuProvider, self).__init__(cfg_opts, log)
        self.__width = self._config_model.width
        self.__height = self._config_model.height
        self.__logger = log
        self.buffer_name = 'bootmenu_buffer'
        driver_cfg = None
        ...
        self.__driver = DriverFactory.create(driver_cfg, self.__logger)
        self.__driver.register(buffer_name=self.buffer_name)
        try:
            self.__driver.start()
        except RuntimeError as ex:
            self.__logger.error('Dont stat threading !')
        self.__adapter = \
            _ContextInstance.get(self.__driver, logger=self.__logger, buffer_name=self.buffer_name,
