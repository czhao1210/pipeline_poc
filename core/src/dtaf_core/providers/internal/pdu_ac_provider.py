...
class PduAcProvider(AcPowerControlProvider, PduDriver):
    """
    This class is used to provide interfaces for the ac power controlling on PI Control box.
    """

    def __init__(self, cfg_opts, log):
        super(PduAcProvider, self).__init__(cfg_opts, log)

    def __enter__(self):
        return super(PduAcProvider, self).__enter__()

    def __exit__(self, exc_type, exc_val, exc_tb):
        super(PduAcProvider, self).__exit__(exc_type, exc_val, exc_tb)
    ...
    def ac_power_on(self, timeout=None):
        ...
        driver_cfg = ConfigurationHelper.get_driver_config(provider=self._cfg,
                                                           driver_name=r"pdu")
        with DriverFactory.create(cfg_opts=driver_cfg, logger=self._log) as s_w: #type: PduDriver
            try:
                if s_w.ac_power_on(timeout):
                    return True
                return False
            except Exception as ex:
                self._log.error("pudssh ac power on error:{}".format(ex))
    ...
    def ac_power_off(self, timeout=None):
        ...
        with DriverFactory.create(cfg_opts=driver_cfg, logger=self._log) as s_w: #type: PduDriver
            try:
                if not s_w.ac_power_off(timeout):
                    return False
                return True
            except Exception as ex:
                self._log.error("pdussh ac power off error:{}".format(ex))
                raise DriverIOError(ex)
    ...
    def get_ac_power_state(self, timeout=None):
        ...
        with DriverFactory.create(cfg_opts=driver_cfg, logger=self._log) as s_w: #type: PduDriver
            try:
                return s_w.get_ac_power_state(timeout)
            except Exception as ex:
                self._log.error("pdussh ac power on error:{}".format(ex))
                raise DriverIOError(ex)
