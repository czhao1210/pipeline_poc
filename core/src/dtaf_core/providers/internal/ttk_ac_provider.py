...
from dtaf_core.drivers.internal.ttk_driver import TTKDriver
from dtaf_core.lib.exceptions import DriverIOError
from dtaf_core.providers.ac_power import AcPowerControlProvider
from dtaf_core.lib.configuration import ConfigurationHelper

class TTKAcProvider(AcPowerControlProvider, TTKDriver):
    """
    This class is used to provide interfaces for the ac power controlling using TTK Control box.
    """
    def __init__(self, cfg_opts, log):
        super(TTKAcProvider, self).__init__(cfg_opts, log)
        
    def __enter__(self):
        return super(TTKAcProvider, self).__enter__()

    def __exit__(self, exc_type, exc_val, exc_tb):
        super(TTKAcProvider, self).__exit__(exc_type, exc_val, exc_tb)
    ...
    def ac_power_on(self, timeout=None):
        ...
        :return:    True / None
        :raise TTKError:  Hardware Error from pi driver
        ...
        with DriverFactory.create(cfg_opts=driver_cfg, logger=self._log) as sw:
            try:
                ...
                self._log.error("ttk ac power on error:{}".format(ex))
                raise
    ...
    def ac_power_off(self, timeout=None):
        ...
        with DriverFactory.create(cfg_opts=driver_cfg, logger=self._log) as sw:
            try:
                ...
                self._log.error("TTK ac power off error:{}".format(ex))
                raise
