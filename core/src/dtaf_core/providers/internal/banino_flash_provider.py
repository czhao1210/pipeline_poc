...
class BaninoFlashProvider(FlashProvider, BaninoDriver):
    def __init__(self, cfg_opts, log):
        ...
        """
        super(BaninoFlashProvider, self).__init__(cfg_opts, log)
        self.__image_path = self._config_model.driver_cfg.image_path
        self.__image_name = self._config_model.driver_cfg.image_name
        self.__image_path_bmc = self._config_model.driver_cfg.image_path_bmc
        self.__image_name_bmc = self._config_model.driver_cfg.image_name_bmc

    def __enter__(self):
        return super(BaninoFlashProvider, self).__enter__()

    def __exit__(self, exc_type, exc_val, exc_tb):
        super(BaninoFlashProvider, self).__exit__(exc_type, exc_val, exc_tb)
    ...
    def flash_image_bmc(self, path=None, image_name=None):
        ...
        with DriverFactory.create(cfg_opts=driver_cfg, logger=self._log) as sw:
            ...
            try:
                if sw.chip_flash_bmc(path, image_name):
                    return True
            except Exception:
                self._log.error("Couldn't Detect The BMC Flash Chip In Platform")
                raise

    def update_configuration(self, update_cfg_opts, update_log):
        super(BaninoFlashProvider, self).update_configuration(update_cfg_opts=update_cfg_opts, update_log=update_log)
